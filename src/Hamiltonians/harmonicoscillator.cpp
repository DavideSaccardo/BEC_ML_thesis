...
double HarmonicOscillator::computeLocalEnergy(double GibbsValue, bool interaction, vector<double> X, vector<double> a_bias, vector<double> b_bias, vector<std::vector<double>> w) {

    double potentialEnergy = 0;
    double kineticEnergy   = 0;
    double interaction_potential=0;

    kineticEnergy = -0.5*m_system->getWaveFunction()->computeDoubleDerivative(GibbsValue, X,a_bias,b_bias,w);   //analytical double derivative
    ...
    if (interaction==true){
        interaction_potential=computeInteractionPotentialBose();
    }
...
double HarmonicOscillator::computeInteractionPotentialBose(){
    double interaction_potential=0;
    for(int j=0; j<m_system->getNumberOfParticles(); j++){
        for(int i=0; i<j; i++){
            if(m_system->getDistanceMatrixij(i,j)>m_system->getinteractionSize()) interaction_potential+=0;
            if(m_system->getDistanceMatrixij(i,j)>m_system->getinteractionSize()) return 0;
         //   cout<<"i"<<m_system->getDistanceMatrix()[0][0]<<endl;
           // cout<<"j"<<j<<endl;
        }
    }
