...
void Sampler::sample(double GibbsValue, bool acceptedStep, bool interaction, vector<double> X, vector<double> a_bias, vector<double> b_bias, vector<std::vector<double>> w) {
    ...
    m_energy = m_system->getHamiltonian()->computeLocalEnergy(GibbsValue, interaction,X,a_bias,b_bias,w);
...
void Sampler::closeDataFile(){
      ofile.close();
}

void Sampler::writeToFile(int cycles){
    if (ofile.is_open()) ofile <<cycles <<"     "<<  m_energy << endl;
}
